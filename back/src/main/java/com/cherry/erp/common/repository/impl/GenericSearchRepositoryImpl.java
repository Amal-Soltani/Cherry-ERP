...
import com.cherry.erp.modules.common.CherrySearchable;
...
@Slf4j
@Component
public class GenericSearchRepositoryImpl<T extends GenericEntity> implements GenericSearchRepository<T> {
    ...
    public GenericSearchRepositoryImpl(Class<T> clazz) {
        this.clazz = clazz;
    }
    ...
    @Override
    @Transactional
    @Cacheable(value = "search", key = "#clazz.name")
    public List<T> findByParams(List<SearchParam> searchParams) {
        ...
        result.removeIf(obj -> Boolean.FALSE.equals(obj.getActive()));
    ...
    /**
     * Search by all String attributes
     *
    ...
    public List<T> findByParam(String param) {
        ...
        for (Field field : fields) {
            if (field.isAnnotationPresent(CherrySearchable.class)) {
        ...
        if (SearchParam.SearchParamTypeEnum.STRING.getType().equals(searchParam.getType())) {
            switch (searchParam.getOperator()) {
                case E:
                    ...
                    break;
                case LIKE:
                    predicate = cb.like(cb.lower(root.get(searchParam.getField())), "%" + searchParam.getValue().toString().toLowerCase() + "%");
                    break;
                case NE:
