...
var PivotDataObject = (function() {
  ...
  PivotData = (function() {
    function PivotData(input, opts) {
      var ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, reffiltersData;
      if (opts == null) {
        opts = {};
      }
      ...
      this.aggregator = (ref = aggregators[opts.aggregatorName](opts.vals)) != null ? ref : aggregatorTemplates.count()();
      ...
      this.filtersData = (reffiltersData = opts.filtersData) != null ? reffiltersData : {};;
      this.tree = {};
      this.usePivot = opts.usePivot;
      this.useDrill = opts.useDrill;
      this.rowKeys = [];
      this.colKeys = [];
      this.rowTotals = {};
      this.colTotals = {};
      ...
      PivotData.forEachRecord(this.input, this.derivedAttributes, (function(_this) {
        return function(record) {
          if (_this.filter(record, _this.filtersData)) {
  ...
  renderFlatTable = function(options, input){

    let colKeys = options.cols;
    let rowKeys = options.rows;
   ...
   * ******************************************************************
   ********************************************************************/
  $.fn.pivot = function(input, inputOpts, locale, pivotDataToreturn) {
    ...
    if(inputOpts.rendererName) {
      defaults.renderer = $.pivotUtilities.renderers[inputOpts.rendererName]
    }

    if(inputOpts.dataClassName && inputOpts.dataClassName === 'SubtotalPivotData') {
      defaults.dataClass = $.pivotUtilities.SubtotalPivotData
    }
    ...
    try {
      ...
      //console.log('Draw ' + opts.rendererName + ' using function = '+ opts.aggregatorName);

      try {

        if( opts.usePivot ){
          pivotData = new opts.dataClass(input, opts);
          result = opts.renderer(pivotData, opts.rendererOptions, this);
        } else {
          if( opts.rendererName !== 'Table' ) {
            pivotData = { data : input, cols: opts.cols, rows: opts.rows, rowOrder: opts.rowOrder, colOrder: opts.colOrder};
            if( opts.onlyData ) {
              return pivotData;
            }
            result = opts.renderer(pivotData, opts.rendererOptions, this);
          } else {
            result =  renderFlatTable(opts, input);
          }
        }

        if (opts.onRefresh) {
          opts.onRefresh(pivotData);
        }
      } catch (error) {
        e = error;
   ...
   */
  $.fn.pivotUI = function(input, inputOpts, overwrite, locale, idDiv) {
    ...
    try {
      ...
      rendererControl.css('width' , '220px');
      renderer = $("<select>")
      ...
      refreshDelayed = (function(_this) {
        return function() {
          ...
          subopts = {
            ...
            cols: [],
            rows: [],
            usePivot : opts.usePivot,
            useDrill : opts.useDrill,
          ...
          if (numInputsToProcess !== 0) {
            ...
            for (x = t = 0, ref5 = numInputsToProcess; 0 <= ref5 ? t < ref5 : t > ref5; x = 0 <= ref5 ? ++t : --t) {
              newDropdown = $("<select>").addClass('pvtAttrDropdown')
                .addClass('form-control')
                .append($("<option>"))
                .bind("change", function() {
                  return refresh();
          ...
          pivotTableDiv = $("<div>")
            .attr("id", idDiv)
            .html('DIV data')
            .addClass('pivotTableDivClass')
            .css('min-height','150px');

          pivotTable.append(pivotTableDiv);
          pivotTableDiv.pivot(materializedInput, subopts, {});
